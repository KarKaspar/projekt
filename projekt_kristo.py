...
def krüpteerminie():
    if valik.get() == "Tekst":
        user = nimi.get()
        nimi.delete(0, tk.END)
        nimi.insert(0, caesar_krüpti(user))
    elif valik.get() == "Fail":
        content = faili_avamine.get("1.0", tk.END).strip()
        faili_avamine.delete("1.0", tk.END)
        faili_avamine.insert(tk.END, caesar_krüpti(content))
...
def dekrüpteerminie():
    if valik.get() == "Tekst":
        ...
        content = faili_avamine.get("1.0", tk.END).strip()
        faili_avamine.delete("1.0", tk.END)
        faili_avamine.insert(tk.END, caesar_dekrüpti(content))

# avab faili ja kuvab selle tekstiväljale
def ava_fail():
    if valik.get() == "Fail":
        ...
        if failitee:
            with open(failitee, "r") as fail:
                content = fail.read()
                faili_avamine.delete(1.0, tk.END)
                faili_avamine.insert(tk.END, content)

# raadionupud teksi ja faili eraldi  sisselülitamiseks
def val():
    if valik.get() == "Tekst":
        nimi.config(state="normal")
        faili_avamine.config(state="disabled")
    elif valik.get() == "Fail":
        nimi.config(state="disabled")
        faili_avamine.config(state="normal")
...
nimi.place(x=70, y=100, width=300)

# loome nupu
nupp = ttk.Button(raam, text="Krüpteeri", command=krüpteerminie)
nupp.place(x=70, y=350, width=150)
nupp = ttk.Button(raam, text="Dekrüpteeri", command=dekrüpteerminie)
nupp.place(x=230, y=350, width=150)

# Raadionupp
def val():
    print(valik.get())

valik = tk.StringVar(value="Tekst")
...
faili_avamine = tk.Text(aken, wrap=tk.WORD)
faili_avamine.place(x=70, y=190, width=300, height=100)

# faili avamise nupp
avamis_nupp = tk.Button(aken, text="Ava Fail", command=ava_fail)
avamis_nupp.place(x=70, y=300, width=150)
